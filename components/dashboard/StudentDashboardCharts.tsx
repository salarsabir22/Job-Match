  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
      <div className="rounded-xl border border-border p-4 bg-card">
        <p className="font-data text-[9px] tracking-widest uppercase text-muted-foreground mb-1">Swipes over time</p>
        <p className="font-body text-sm text-foreground mb-3">Applications vs saved (last 30 days)</p>
          ...
          {emptyActivity ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={activity} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="saved" name="Saved" stroke="#6b7280" strokeWidth={2} strokeDasharray="4 3" dot={false} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      <div className="rounded-xl border border-border p-4 bg-card">
        <p className="font-data text-[9px] tracking-widest uppercase text-muted-foreground mb-1">Matches</p>
        <p className="font-body text-sm text-foreground mb-3">New matches per day (last 30 days)</p>
        <div className="h-[240px] w-full">
          {emptyMatches ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={matchesSeries} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="matches" name="Matches" stroke="#1e3a5f" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
